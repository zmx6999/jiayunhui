...
- (void)buildMultipartFormDataPostBody
{
	...
	NSUInteger i=0;
    if (self.isIM) {
        [self appendPostString:[NSString stringWithFormat:@"Content-Disposition: form-data;"]];
        for (NSDictionary *val in [self postData]) {
            [self appendPostString:[NSString stringWithFormat:@"%@=\"%@\"",[val objectForKey:@"key"],[val objectForKey:@"value"]]];
            i++;
            if (i == [[self postData] count]) {
                //如果是最后一行
